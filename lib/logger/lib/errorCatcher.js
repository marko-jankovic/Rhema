...
const errorFile = {
    ...
    async updateLineByLine (filePath, data, callback) {
        ...
        const output = await utils.fs.createWriteStream(filePath, { flags: 'a' });
    ...
    prepareFile (filePath, callback) {
        ...
        utils.fs.unlinkFile(tempPath, async () => {
...
module.exports = (options) => {
    ...
    setInterval(() => {
        ...
        if (Object.keys(errorFile.$cache).length) {
            errorFile.updateFile(options.export, errorFile.$cache, (err) => {
                if (err) {
