...
const express = require('@rhema/express');
const Dispatcher = require('@rhema/dispatcher');
const Middleware = require('@rhema/middleware');
const services = require('@rhema/services');
const Request = require('@rhema/http/request');
const Response = require('@rhema/http/response');
const ControllerResolver = require('@rhema/controller/controllerResolver');
const Profiler = require('@rhema/profiler');
const Server = require('@rhema/server');
const utils = require('@rhema/utils');
const vhost = require('vhost');
const os = require('@rhema/os');
...
class App {
    constructor (config, appCallback) {
        ...
        const startServer = () => {
            ...
            server(app, (serverInstance, gracefulRestart) => {
                ...
                appCallback(app, config[utils.env.get('NAME')], servicesInstance);
            });
        };
    ...
    bootstrapApp (config) {
        ...
        if (config[utils.env.get('NAME')].app.vhosts) {
            this.registerApps(expressApp, config[utils.env.get('NAME')]);
        }
