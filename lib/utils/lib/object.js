    ...
    merge (target, source = {}, immutable) {
        ...
        Object.keys(source).forEach((key) => {
            ...
            if (this.isObject(source[key])) {
                ...
                if (!(key in target)) {
                    if (immutable) {
                        data = Object.assign(this.deepClone(target), data, this.deepClone({ [key]: source[key] }));
                    } else {
                        target = Object.assign(target, { [key]: source[key] });
                    }
                } else {
                    if (immutable) {
                        data = this.merge(target[key], source[key], true);
                    } else {
                        this.merge(target[key], source[key], false);
                    }
                }
            } else {
                if (immutable) {
                    data = Object.assign(this.deepClone(target), data, { [key]: this.deepClone(source[key]) });
                } else {
