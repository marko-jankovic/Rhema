...
const fs = require('fs');
const mkdir = require('./filesystem/fs').mkdir;
...
module.exports = {
    ...
    async updateFileSync (filePath, data, field) {
        const content = {};
    ...
    loadToJson (filePath) {
        ...
        const fileName = filesystem.getFileName(readFile);
        ...
        switch (extname(filePath)) {
            case '.json':
                ...
                if (!jsonFile) {
                    throw new Error(`Cannot parse ${fileName} config file!`)
                }
            ...
            case '.yml':
                ...
                try {
                    ...
                    throw new Error(`Cannot parse ${fileName} config file - ${e.message}!`)
                }
            case '.ini':
