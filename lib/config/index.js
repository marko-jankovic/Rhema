...
class Config {
    ...
    createConfig (options) {
        ...
        config.apps.main = utils.object.deepMerge(this.baseConfig, config.apps.main);

        // flag just for unit tests
        if (options.testConfig) {
            config.apps.main = utils.object.deepMerge(config.apps.main, options.testConfig);
        }
    ...
    createSubdomainsConfig (mainConfig, subdomainsDir) {
        ...
        _.each(subdomainConfig, (object, cobrand) => {
            // config is valid only if it has vhosts
            if (object.app && object.app.vhosts && object.app.vhosts.length > 0) {
                // merge current subdomain (cobrand) and main
                subdomainConfig[cobrand] = utils.object.deepMerge(config, subdomainConfig[cobrand]);
                ...
                // prevent starting application if subdomain (cobrand) is not valid
                throw new Error(`${cobrand} subdomain does not have vhosts`);
            }
        });
    ...
    mergeEnvConfig (config) {
        ...
        if (!utils.env.isLocal()) {
            ...
            if (typeof envConfig !== 'undefined') {
                config = utils.object.merge(config, envConfig);
            } else {
                    ...
                    if (fileExist && isValidExtension) {
                        ...
                        if (options.followDirNames && !rootMatched) {
                            ...
                            if (!obj[fileName] && rootMatched) {
                                filetree = utils.object.merge(filetree, { [fileName]: obj });
                            } else {
                                filetree = utils.object.merge(filetree, obj);
                            }
                        }
                        ...
                        // add to main object
                        if (!obj[fileName]) {
                            utils.object.merge(filetree, { [fileName]: obj }, true);
                        } else {
                            utils.object.merge(filetree, obj, true);
                        }
                    } else {
                        // add to main object
                        utils.object.merge(filetree, utils.json.loadToJson(fileFullPath), true);
                    }
                }
            });
        };
