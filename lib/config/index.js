...
class Config {
    ...
    createConfig (options) {
        ...
        // sync reading all folders from rootDir
        config.apps.main = options.rootDir ? (this.loadConfigSync() || {}) : {};
        ...
        if (options.template) {
            config = this.applyMicroTemplate(config, options);
        }
    ...
    mergeEnvConfig (config) {
        ...
        if (config[envDir]) {
            config[envDir][env] = config[envDir][env] || {};

            // if env (from process) exist in main, merge it with current app
            config = utils.object.deepMerge(config, config[envDir][env]);
        }
    ...
    // recursivly sync reading files from rootDir
    ...
    loadConfigSync () {
        ...
        const readDirSync = (currentPath, filetree) => {
            const isFile = extname(currentPath).includes('.');
            // get file and folder names from config root path
            const files = isFile ? [basename(currentPath)] : utils.fs.readdirSync(currentPath);
            ...
            _.each(files, (file) => {
                ...
                const currentFile = isFile ? currentPath : path.join(currentPath, file);
        ...
        // start recursively reading from rootDir
        readDirSync(options.rootDir, paths);
    ...
    applyMicroTemplate (config, options) {
        const template = (siteConfig) => {
            ...
            regArray.forEach((regValue) => {
                ...
                if (repValue.includes('|')) {
                    ...
                    if (options.helpers && utils.app[helper] && typeof utils.app[helper] === 'function') {
