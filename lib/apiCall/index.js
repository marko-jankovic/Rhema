...
module.exports = function Request (defaultOptions = {}) {
    ...
    return (namespace, moduleOptions = {}, callback, stream = false) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            const options = utils.object.merge(defaultOptions, moduleOptions, true);
            // get url object
            const parseUrl = url.parse(options.uri, true);
            ...
            const fullUrl = (method === 'GET' && queryString) ? `${parseUrl.protocol}//${parseUrl.hostname}${parseUrl.port ? `:${parseUrl.port}` : ''}${parseUrl.pathname}?${queryString}` : options.uri;

            // pass request id
            options.headers['x-request-id'] = registry.get('requestId');

            // set url data to beatle
            beatle.set('apiUrls', {
                [`${parseUrl.hostname}${parseUrl.pathname}`]: {
                    url: fullUrl,
