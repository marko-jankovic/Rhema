...
class RouteDispatcher {
    ...
    routeParam (router, routeName, paramName, regex) {
        // injected prefix to ensure unique params
        const paramPrefix = `${routeName.replace('-', '_')}_`;
    ...
    prepareRouteOptions (route, method, path) {
        ...
        if (Array.isArray(this.config.settings.formats)) {
    ...
    prepareRouterConfig (routes, callback) {
        _.each(routes, (route, routeName) => {
            ...
            if (route.handler.indexOf(':') !== -1 && _.isEmpty(route.methods)) {
    ...
    setRoutes (router, routeInvoked, controllerResolver) {
        const setHandler = (route, method, path) => {
            const routeOptions = this.prepareRouteOptions(route, method, path);
            ...
            if (!controllerResolver) {
                ...
                router[routeOptions.method](
                    ...
                    (req, res, next) => {
                        ...
                        // </form>
                        const method = req.method || route.action || routeOptions.method;
        ...
        this.prepareRouterConfig(this.config.routes, (route, routeName) => {
            if (utils.env.isActive(route)) {
                ...
                Object.keys(route.params).forEach((paramName) => {
                    this.routeParam(router, routeName, paramName, route.params[paramName]);
                });
